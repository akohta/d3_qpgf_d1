...
{
  int d3hm_qpgf_d1_fd_qG(double complex *qG,double *r,double eps,QPDT *qd);
  int d3hm_qpgf_d1_ew_qG(double complex *qG,double *r,double eps,QPDT *qd);
  ...
  if(bp2>0.0 || bm2>0.0){ // Ewald method
    err=d3hm_qpgf_d1_ew_qG(qG,r,eps,qd);
    if(err<0) err-=20;
  }
  ...
  if(Fp < eps*FN && Fm < eps*FN){ // Fourier
    err=d3hm_qpgf_d1_fd_qG(qG,r,eps,qd);
    if(err<0) err-=10;
  }
  else { // Ewald
    err=d3hm_qpgf_d1_ew_qG(qG,r,eps,qd);
...
{
  int d3hm_qpgf_d1_fd_dqG(double complex *dqG,double *r,double eps,QPDT *qd);
  int d3hm_qpgf_d1_ew_dqG(double complex *dqG,double *r,double eps,QPDT *qd);
  ...
  if(bp2>0.0 || bm2>0.0){ // Ewald method
    err=d3hm_qpgf_d1_ew_dqG(dqG,r,eps,qd);
    if(err<0) err-=20;
  }
  ...
  if(Fp < eps*FN && Fm < eps*FN){ // Fourier
    err=d3hm_qpgf_d1_fd_dqG(dqG,r,eps,qd);
    if(err<0) err-=10;
  }
  else { // Ewald
    err=d3hm_qpgf_d1_ew_dqG(dqG,r,eps,qd);
...
{ // Fourier domain summation
  int d3hm_qpgf_d1_fd_qG_sd(double complex *qG,double *r,double eps,QPDT *qd);
  ...
  err=d3hm_qpgf_d1_fd_qG_sd(qG,rt,eps,qd);
  if(l!=0){
    tmp=(double)l*qd->kx*qd->d;
    c=cos(tmp)-I*sin(tmp);
    *qG*=c;
...
int d3hm_qpgf_d1_ew_qG(double complex *qG,double *r,double eps,QPDT *qd)
{ // Ewald method
  int d3hm_ewald_qG1_qG(double complex *qG1,double *r,double eps,double veps,QPDT *qd);
  int d3hm_ewald_qG2_qG(double complex *qG1,double *r,double eps,double veps,QPDT *qd);
  ...
  // qG1
  err1=d3hm_ewald_qG1_qG(&tqG,r,eps,veps,qd);
  *qG=tqG;
  if(err1==-1) return -1;

  // qG2
  err2=d3hm_ewald_qG2_qG(&tqG,r,eps,veps,qd);
  *qG+=tqG;
  if(err2==-1) return -2;
  if(err2==-2) return -3; // beta_n==0.0
...
int d3hm_ewald_qG2_qG(double complex *qG2,double *r,double eps,double veps,QPDT *qd)
{
  int sum_expint_qG(double complex *ret0,double ro2,double veps,double bn2,double eps);
  ...
  eiaxN=cos(arg)+I*sin(arg);
  bn2=k2-aN*aN; if(bn2==0.0) return -2;
  err=sum_expint_qG(&tc0,ro2,veps,bn2,eps);  if(err<0) return -1;
  *qG2=eiaxN*tc0;
...
int d3hm_qpgf_d1_fd_dqG(double complex *dqG,double *r,double eps,QPDT *qd)
{ // Fourier domain summation
  int d3hm_qpgf_d1_fd_sd_dqG(double complex *dqG,double *r,double eps,QPDT *qd);
  ...
  err=d3hm_qpgf_d1_fd_sd_dqG(dqG,rt,eps,qd);
  if(l!=0){
    tmp=(double)l*qd->kx*qd->d;
    c=cos(tmp)-I*sin(tmp);
    dqG[0]*=c;
...
int d3hm_qpgf_d1_ew_dqG(double complex *dqG,double *r,double eps,QPDT *qd)
{ // Ewald method
  int d3hm_ewald_qG1_dqG(double complex *dqG1,double *r,double eps,double veps,QPDT *qd);
  int d3hm_ewald_qG2_dqG(double complex *dqG1,double *r,double eps,double veps,QPDT *qd);
  ...
  // qG1
  err1=d3hm_ewald_qG1_dqG(tdqG,r,eps,veps,qd);
  dqG[0]=tdqG[0];
  dqG[1]=tdqG[1];
  dqG[2]=tdqG[2];
  if(err1==-1) return -1;
  ...
  // qG2
  err2=d3hm_ewald_qG2_dqG(tdqG,r,eps,veps,qd);
  dqG[0]+=tdqG[0];
  dqG[1]+=tdqG[1];
  dqG[2]+=tdqG[2];
  if(err2==-1) return -2;
...
int d3hm_ewald_qG2_dqG(double complex *dqG2,double *r,double eps,double veps,QPDT *qd)
{
  int sum_expint(double complex *ret0,double complex *ret1,double ro2,double veps,double bn2,double eps);
